...
class Metrics:
    ...
    def __init__(self,
        ...
        self.inference_totals = [0. for i in range(len(self.metrics))]
...
class SparseBinarySemanticSegmentationAccuracy(Metric):
    ...
    def update(self,
        ...
        number_correct = (predictions == labels)
        self.number_correct += (predictions == labels).sum()
        self.number_pixels += len(predictions)
...
class SparseBinarySemanticSegmentationMetrics(Metrics):
    ...
    def __init__(self):
        ...
        self.val_metrics = self.metrics.clone(prefix='val_')
        self.test_metrics = self.metrics.clone(prefix='test_')
        self.inf_metrics = self.metrics.clone(prefix='inference_')
        self.logger.info(f"Constructed metric collection with items {_dict}.")
        # running totals for a given epoch
        self.reset_metrics()
        self.metric_names = ['accuracy','dice_score']
    ...
    def inference_metrics(self,
        ...
        predictions = predictions.F.to(self.device)
        targets = targets.type('torch.IntTensor').to(self.device)
        results = self.inf_metrics(predictions, targets)
        metrics = []
        for ii, item in enumerate(results.keys()):
            metrics.append(results[item])
        self.inference_totals.append(results)
        return metrics
